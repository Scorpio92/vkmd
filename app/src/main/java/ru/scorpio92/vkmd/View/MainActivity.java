...
import ru.scorpio92.vkmd.Adapters.TracksListAdapter;
import ru.scorpio92.vkmd.Interfaces.ActivityWatcher;
import ru.scorpio92.vkmd.Interfaces.FooterFragmentWatcher;
import ru.scorpio92.vkmd.Interfaces.OperationsCallbacks;
import ru.scorpio92.vkmd.Operations.ScanForSavedTracks;
import ru.scorpio92.vkmd.Operations.UpdateDownloadInfo;
...
public class MainActivity extends Activity implements OperationsCallbacks, TracksListAdapter.TracksListAdapterCallbacks, FooterFragmentWatcher {
    ...
    void initAdapter(TrackList trackList) {
        try {
            if(trackList.getAllTracks().size() > 0) {
                ...
                Log.w(LOG_TAG, "initAdapter, relogin");
    ...
    void initAndStartBindingWithAudioService(final boolean relogin) {
        sConn = new ServiceConnection() {
            public void onServiceConnected(ComponentName name, IBinder binder) {
                ...
                if(!relogin) {
                    ...
                    try {
                        Log.w(LOG_TAG, "unbindService, relogin");
                        unbindService(sConn);
                        sConn = null;
                        audioService = null;
                    } catch (Exception e) {
    ...
    public void onUpdateDownloadInfoComplete(int count, int action) {
        if(action == UpdateDownloadInfo.ACTION_INSERT) {
            ...
            String folder = CommonUtils.getStringSetting(MainActivity.this, Settings.SETTING_VKMD_FOLDER_KEY, DownloadService.DEFAULT_DOWNLOAD_FOLDER);
            Log.w(LOG_TAG, "SETTING_VKMD_FOLDER_KEY: " + folder);
